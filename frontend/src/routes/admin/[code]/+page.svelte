...
{:else}
	<div class="flex h-screen overflow-hidden bg-hacksu-grey">
		<aside class="flex w-56 shrink-0 flex-col overflow-y-auto border-r border-white/10">
			<div class="border-b border-white/10 bg-hacksu-grey px-3 py-2 text-xs font-semibold uppercase tracking-wider text-gray-400">
				Participants
			</div>
			{#each Object.entries(result.participants) as [token, p] (token)}
				<button
					type="button"
					class={`w-full px-3 py-2 text-left hover:bg-white/10 ${selected === token ? 'bg-white/15 font-semibold' : ''}`}
					onclick={() => { selected = token; activeTab = 'html'; }}
				>
					<div class="truncate text-sm text-gray-200">{p.name}</div>
		...
		<div class="flex flex-1 flex-col overflow-hidden">
			{#if selectedParticipant}
				<div class="flex shrink-0 items-center gap-3 border-b border-white/10 px-4 py-2">
					<span class="font-medium text-gray-100">{selectedParticipant.name}</span>
					<span class="text-sm text-gray-400">Penalty: {formatPenalty(selectedParticipant.penalty_ms)}</span>
					{#if selectedParticipant.submitted_at}
						<span class="rounded bg-hacksu-green px-1.5 py-0.5 text-xs text-white">submitted</span>
					{/if}
				</div>

				{#if !selectedParticipant.final_html && !selectedParticipant.final_css}
					<div class="flex flex-1 items-center justify-center text-gray-500">Not submitted</div>
				{:else}
					<div class="flex shrink-0 gap-1 border-b border-white/10 px-2 py-1">
						<button
							type="button"
							class={`cursor-pointer rounded-t border border-white/20 px-3 py-1 text-sm ${activeTab === 'html' ? 'bg-white/15 font-bold text-white' : 'bg-transparent text-gray-400'}`}
							onclick={() => (activeTab = 'html')}
						>HTML</button>
						<button
							type="button"
							class={`cursor-pointer rounded-t border border-white/20 px-3 py-1 text-sm ${activeTab === 'css' ? 'bg-white/15 font-bold text-white' : 'bg-transparent text-gray-400'}`}
							onclick={() => (activeTab = 'css')}
						>CSS</button>
					</div>
					<div class="grid flex-1 grid-cols-2 overflow-hidden">
						<div class="overflow-hidden border-r border-white/10">
							<Editor
								language={activeTab}
